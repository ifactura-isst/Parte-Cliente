...
public class SubastaDAOImpl implements SubastaDAO {
	...
	@Override
	public ArrayList<String> getCustomersOfSubasta(long subastaId) {
		EntityManager em = EMFService.get().createEntityManager();
		try {
			Subasta subasta = em.find(Subasta.class, subastaId);
			ArrayList<String> usersApuntados = new ArrayList<String>(subasta.getUsuariosApuntados());
			return usersApuntados;
		} finally {
			em.close();
		}
	}
	...
	@Override
	public void updateUsersApuntados(long subastaId, int userApuntados) {
		EntityManager em = EMFService.get().createEntityManager();
		try {
			Subasta subasta = em.find(Subasta.class, subastaId);
			subasta.setuserApuntados(userApuntados);
	...
	@Override
	public void updateState(long subastaId, boolean state) {
		EntityManager em = EMFService.get().createEntityManager();
		try {
			Subasta subasta = em.find(Subasta.class, subastaId);
			subasta.setState(state);
	...
	@Override
	public void updateCustomers(long subastaId, ArrayList<String> customers) {
		EntityManager em = EMFService.get().createEntityManager();
		try {
			Subasta subasta = em.find(Subasta.class, subastaId);
			subasta.setCustomers(customers);
